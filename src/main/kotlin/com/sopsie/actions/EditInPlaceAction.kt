...
private val LOG = logger<EditInPlaceAction>()

/**
 * Action to open an editable temp file with decrypted content.
 * Saves automatically encrypt back to the original file.
 */
class EditInPlaceAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (!file.isInLocalFileSystem) {
            showWarning(project, "Can only edit local files")
            return
        }
        ...
        LOG.debug("EditInPlaceAction: Opening edit-in-place for ${file.path}")
